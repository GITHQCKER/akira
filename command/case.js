/*
# akira - UserBot
# Copyright (C) 2021 Akira-Bot Created By AkiraYT
#
# This file is a part of < https://github.com/Akirachan99/akira/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/Akirachan99/akira/blob/v1/LICENSE/>.
*/ 
...
//start script
			async function starts() {
				...
				akira.version = [2, 2143, 3]  //jika ada update dari WhatsApp web silahkan ubah
				akira.logger.level = 'warn'
				console.log(color(figlet.textSync('akira-Bot', {
					font: 'Standard',
				...
				akira.on('qr', () => {
					console.log(color('[','white'), color('!','red'), color(']','white'), color('SCAN QR MASK 15 SECONDS, AND SUBSCRIBE YOUTUBE AKIRA YT CHANNEL'))
					})
			fs.existsSync(`./settings/${session_name}`) && akira.loadAuthInfo(`./settings/${session_name}`)
			akira.on('connecting', () => {
			...
			fs.writeFileSync(`./settings/${session_name}`, JSON.stringify(akira.base64EncodedAuthInfo(), null, '\t'))
...
//greetings
				akira.on('group-participants-update', async(anu) => {
					try {
						const sendButLoc = async (id, text1, desc1, gam1, but = [], options = {}) => {
							const mediaxxaaaa = await akira.prepareMessage(id, gam1, MessageType.location, {thumbnail: gam1})
							...
							akira.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
						}
				...
				akira.on('chat-update', async (mek) => {
					try {
						...
						const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = akira.user.phone
						const botNumber = akira.user.jid
						const ownerNumber = [`${ownernumber}@s.whatsapp.net`] 
						const isGroup = from.endsWith('@g.us')
						const totalchat = await akira.chats.all()
						const sender = mek.key.fromMe ? akira.user.jid : isGroup ? mek.participant : mek.key.remoteJid
						const isOwner = mek.key.fromMe ? akira.user.jid : ownerNumber.includes(sender)
						const conts = mek.key.fromMe ? akira.user.jid : akira.contacts[sender] || { notify: jid.replace(/@.+/, '') }
						const pushname = mek.key.fromMe ? akira.user.name : conts.notify || conts.vname || conts.name || '-'
   	     ...
   	     let fvn = {key: {participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6281327327914-1613049930@g.us" } : {})},message: { "audioMessage": {"mimetype":"audio/ogg; codecs=opus","seconds":99999,"ptt": "true"}} } 
	        let fgif = {key: {participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6281327327914-1613049930@g.us" } : {})},message: {"videoMessage": { "title":`© ${ownername}`, "h": `Hmm`,'seconds': '99999', 'gifPlayback': 'true', 'caption': `${botname} 🏟️\nBy ${ownername}`, 'jpegThumbnail': thumbnail}}}
			...
			let fvideo = {key: { fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6281327327914-1613049930@g.us" } : {}) },message: { "videoMessage": { "title":`© ${ownername}`, "h": `Hmm`,'seconds': '99999', 'caption': `© ${ownername}`, 'jpegThumbnail': thumbnail}}}
			...
			const checkLimit = (sender) => {
				...
				for (let lmt of _limit) {
					if (lmt.id === sender) {
						...
						if (limitCounts <= 0) return akira.sendMessage(from,`Limit kamu sudah habis`, text,{ quoted: mek})
						akira.sendMessage(from, lang.limitcount(isPremium, limitCounts), text, { quoted : mek})
						found = true
					}
				}
					if (found === false) {
			...
			const isLimit = (sender) =>{ 
				...
				for (let i of _limit) {
					if (i.id === sender) {
						...
						if (limits >= limitawal ) {
							position = true
							akira.sendMessage(from, lang.limitend(pushname), text, {quoted: mek})
							return true
						} else {
							_limit
							position = true
			...
			//function
			const reply = (teks) => {
				akira.sendMessage(from, teks, text, { quoted: mek, thumbnail: thumbnail})
			}
			const isUrl = (url) => {
			...
			const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './sticker' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        akira.sendMessage(to, media, MessageType.sticker,{quoted:mek})
							...
							download(url, filename, async function () {
								...
								if(mime === "image/gif"){
									...
									if(mime.split("/")[0] === "audio"){
										...
										akira.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
			...
			async function sendFileFromUrl(from, url, caption, mek, men) {
				...
				if(mime.split("/")[0] === "audio"){
					...
					return akira.sendMessage(from, await getBuffer(url), type, {caption: caption, quoted: mek, thumbnail: Buffer.alloc(0), mimetype: mime, contextInfo: {"mentionedJid": men ? men : []}})
				}
				...
				const sendButMessage = (id, text1, desc1, but = [], options = {}) => {
					const buttonMessage = {
						...
						akira.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options);
					};
				const sendButLocation = async (id, text1, desc1, gam1, but = [], options = {}) => {
					them = gam1
					mediaxxaa = await akira.prepareMessage(id, them, MessageType.location, {thumbnail: them})
				...
				const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
					them = vid1
					mediaxxaa = await akira.prepareMessage(id, them, MessageType.video)
				...
				const sendButImage = async(id, text1, desc1, vid1, but = [], options = {}) => {
					them = vid1
					mediaxxaa = await akira.prepareMessage(id, them, MessageType.image, {thumbnail: Buffer.alloc(0)})
		        ...
		        await akira.sendMessage(from, `Hmm maap nih gua kick, dilarang share link di group ini`, text , {quoted: mek})
		        akira.groupRemove(from, [kic]).catch((e)=>{reply(`Bot Harus Jadi Admin`)})
		        }
			//game 
			...
			const isakira = checkRegisteredUser(sender)
...
switch (command) {
case 'menu': case 'help': case 'akira':
if (!isakira) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
case 'owner':{
	if (!isakira) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			const vname = akira.contacts[i] != undefined ? akira.contacts[i].vname || akira.contacts[i].notify : undefined
			ini_list.push({
				"displayName": `${ownername}`,
				"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
				})
				}
				hehe = await akira.sendMessage(from, {
					"displayName": `${ini_list.length} kontak`,
					"contacts": ini_list 
					}, 'contactsArrayMessage', { quoted: mek })
					akira.sendMessage(from, `Nih Kak Contact Owner Ku, Cuma Sv Nomor Cewe Ya 🤝`, text, {quoted: hehe})
				}
			break
		...
		for(let i of result.medias){
			if(i.url.includes('mp4')){
				let link = await getBuffer(i.url)
                    akira.sendMessage(from,link,video,{thumbnail: Buffer.alloc(0), quoted: mek,caption: `Instagram •  ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    akira.sendMessage(from,link,image,{thumbnail: Buffer.alloc(0), quoted: mek,caption: `Instagram • ${i.type}`})                  
                }
            }
...
if (!isakira) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					akira.sendMessage(from,noweem , MessageType.document, {mimetype: 'video/mp4',filename: `Tiktok Download.mp4`,quoted: mek})
					})
...
if (!isakira) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					akira.sendMessage(from,weem , MessageType.document, {mimetype: 'video/mp4',filename: `Tiktok Wm.mp4`,quoted: mek})
					})
				}).catch((err) => reply(`Link tidak valid`))
			await limitAdd(sender)
             break 
...
if (!isakira) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					akira.sendMessage(from,audnha , MessageType.document, {mimetype: 'audio/mp4',filename: `Tiktok Music.mp3`,quoted: mek})
					})
				}).catch((err) => reply(`Link tidak valid`))
			await limitAdd(sender)
             break
...
case 'daftar': case 'verify': case 'verif':
			if (isakira) return  reply(lang.regis())
			try {
					ppregis = await akira.getProfilePicture(sender)
				} catch {
...
case 'leave':
			...
			setTimeout( () => {
			akira.sendMessage(from, 'Sayonara👋', text)
			}, 0)
			break
case 'hidetag':
if (!isakira) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
if (!isakira) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			if (!isGroupAdmins) return reply(lang.admin(groupName))
			if (!isBotGroupAdmins) return reply(lang.adminB())
					await akira.groupUpdateSubject(from, `${q}`)
					akira.sendMessage(from, `Sukses Mengubah Nama Grup Menjadi ${q}`, text, { quoted: mek })
			break          
...
if (!isakira) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			if (!isGroupAdmins) return reply(lang.admin(groupName))
			if (!isBotGroupAdmins) return reply(lang.adminB())
					await akira.groupUpdateDescription(from, `${q}`)
					akira.sendMessage(from, `Sukses Mengubah Desk Grup Menjadi ${q}`, text, { quoted: mek })
			break   
case 'kick':
if (!isakira) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			if (!isGroup) return reply(lang.group())
...
case 'bc': case 'broadcast':
			...
			var bcnya3 = await akira.downloadMediaMessage(bcnya2)
					for (let _ of bcnya) {
						akira.sendMessage(_.jid, bcnya3, image, { caption: `*----「  BROADCAST 」----*\n\n${q}` })
						}
						reply('Sukses broadcast')
					} else {
						for (let _ of bcnya) {
							akira.sendMessage(_.jid, `*----「  BROADCAST 」----*\n\n${q}`)
						}
						reply('Sukses broadcast')
					}
					break               
...
try {
	if (!isOwner) return reply(lang.owner(botname))
return akira.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`akiraBot :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
